...
(() => {
  ...
  let hairImage = document.querySelector('#hairImage')
  ...
  const isMeshCheckbox = document.querySelector('#isMeshCheckbox')

  const toggleMeshMode = (state) => {
    hairImage = state
      ? document.querySelector('#meshImage')
      : document.querySelector('#hairImage')

    updateFilter();
  }

  isMeshCheckbox.addEventListener('change', () => {
    toggleMeshMode(isMeshCheckbox.checked)
  });
  ...
  if (tweetButton && hueRotateContoller && brightnessController && saturateController && isMeshCheckbox) {
    tweetButton.addEventListener('click', (event) => {
      event.preventDefault()
      const message = '私のオススメpastakヘアカラーはこちら！皆さんもオススメのヘアカラーをシェアしませんか？'
      const thisUrl = `https://haircolor.pastak.net/?hue=${hueRotateContoller.value}&brightness=${brightnessController.value}&saturate=${saturateController.value}&mesh=${isMeshCheckbox.checked}`
  ...
  if (Object.keys(searchQueries)) {
    ...
    if (isMeshCheckbox && searchQueries['mesh'] !== undefined) {
      const isMesh = !!JSON.parse(searchQueries['mesh'])
      isMeshCheckbox.checked = isMesh
      toggleMeshMode(isMesh)
    }
  }
})()
